...
跨 Agent 使用时，文件读写、DOCX/PDF 解析、询问用户和子 Agent 审查依赖各平台提供的工具。缺少某个工具时，应采用该平台可用的等价方式，工作状态和质量闸门保持不变。
...
`请使用 requirement-tech-docs skill，完整分析当前项目和 docs/需求/xxx.docx，先和我确认技术方案，再生成 solution 和 implementation。`
...
首次使用时观察 Agent 是否先读取项目代码和需求，是否在存在真实技术分歧时进入不限轮次的决策流程，是否在用户授权前保持文档不落盘，是否在最终写入前执行代码适配、旧数据、协议、Redis、MySQL、消息和 Lua 等对抗检查。
