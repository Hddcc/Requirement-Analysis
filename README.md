...
- `implementation.md`：说明精确到文件、类型、函数、完整调用链、配置、数据读写、异步处理、测试和联调的实现步骤。

Skill 不预设语言、框架、数据组件、异步机制和外部平台。每次执行都会以当前仓库源代码为准，识别实际技术栈、服务挂载、代码分层、命名方式、错误处理、数据结构和测试习惯。
...
正式写入前会进行对抗式审查。Skill 会先根据代码和候选方案建立技术组件清单，再检查实际适用的协议或通信契约、数据与历史状态、缓存、原子操作、异步处理、幂等、事务、配置、前端及客户端联调、测试和发布回滚。项目没有使用且方案没有计划引入的技术不会进入检查清单。发现会影响旧代码或旧数据的问题时，会先交给用户决定。

## 支持的需求来源

需求文件可以是 Markdown、TXT、Word、PDF、Excel、CSV、PPT、图片或多个文件的组合，也可以是 Agent 能访问的网页和在线文档。Skill 会根据格式选择文本读取、结构化解析、表格解析、页面渲染、视觉检查或 OCR，并保留会影响需求含义的标题、列表、表格、批注、原型和图片关系。
...
首次调用时可以提供一个或多个需求文件路径、目录或网页链接，以及目标文档目录、后端负责范围和已知限制。需求可以使用 MD、TXT、Word、PDF、表格或图片等格式。仓库中能够确认的信息无需重复说明，Agent 会先分析代码。
